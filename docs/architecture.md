...
не нужен, потому что у сервера нет зависимостей. БД (`./data:/data`)
и демки (`./sites:/sites`, read-only) монтируются с хоста — и то и другое
обновляется без пересборки образа, подробности в [deploy.md](deploy.md).
