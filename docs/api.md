...
Все эндпоинты живут под `/api/`. Остальные пути отдают статику:
сначала `/sitemap.xml`, потом демки из `sites/` (роуты `/<slug>/`), потом
пререндеренный портфолио (неизвестный путь — 404).
...
отсортированы по дате пуша. Кэшируется в базе на 6 часов; если GitHub
...
Слаги смонтированных демок — папок в `sites/` с `index.html` внутри.
Каждый слаг доступен как роут `/<slug>/`. Фронт может подсвечивать
на карточках проектов живые демки.
