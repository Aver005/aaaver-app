...
Локальная база одной командой (нужен Docker):

```sh
bun run dev:db          # Postgres 17 на 127.0.0.1:5433, том отдельный
```

и в `.env.local` (Bun читает его поверх `.env`, в git он не попадает):

```
DATABASE_URL=postgres://aaaver:aaaver@127.0.0.1:5433/aaaverdb
TELEGRAM_BOT_TOKEN=
```

Пустой токен намеренно: в деве сообщения с формы копятся в базе, а не
улетают в живой Telegram.
...
[site-release.yml](.github/workflows/site-release.yml). Детали
и правила сборки демок — в [docs/deploy.md](docs/deploy.md).
